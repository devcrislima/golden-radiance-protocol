      ...
      <div className="container mx-auto px-4 relative">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative">
            {/* Connecting line */}
            <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-primary/30 via-primary/50 to-primary/30 hidden md:block" />
              ...
              {steps.map((step, index) => (
                ...
                >
                  ...
                  <div className={`flex-1 ${index % 2 === 0 ? "md:text-right" : "md:text-left"}`}>
                    <div className={`p-6 rounded-xl bg-card border border-border hover:border-primary/30 transition-colors shadow-soft ${
                      index % 2 === 0 ? "md:ml-auto" : "md:mr-auto"
                    } max-w-md`}>
                      <span className="text-primary font-serif text-3xl font-medium">{step.number}</span>
                      <h3 className="font-serif text-xl font-medium mt-2 mb-2 text-foreground">{step.title}</h3>
                      <p className="text-muted-foreground">{step.description}</p>
                    </div>
                  </div>

                  {/* Dot */}
                  <div className="hidden md:flex w-12 h-12 rounded-full bg-primary/10 border-2 border-primary items-center justify-center shrink-0 z-10 bg-background">
          ...
          <div className="mt-16 flex flex-col sm:flex-row gap-6 justify-center items-center">
            <div className="flex items-center gap-3 px-6 py-4 rounded-xl bg-card border border-border shadow-soft">
              <Clock className="w-6 h-6 text-primary" />
              <div>
                <p className="text-sm text-muted-foreground">Tempo em clínica</p>
                <p className="font-medium text-foreground">30 a 40 minutos</p>
              </div>
            </div>
            <div className="flex items-center gap-3 px-6 py-4 rounded-xl bg-card border border-border shadow-soft">
              ...
              <div>
                ...
                <p className="font-medium text-foreground">Permanece até 8 horas</p>
              </div>
            </div>
          </div>
